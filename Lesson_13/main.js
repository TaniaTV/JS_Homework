...
function createUsers(arr) {
    var allInform = JSON.parse(localStorage.getItem('i'))
    var num = 1;
    mainContainer.innerHTML += '<button class="not_active active" onclick = "onClick(' +JSON.parse(localStorage.getItem('0').id + ')" id = "' + JSON.parse(localStorage.getItem('0').id + '">' + 'User ' + num + '</button>' +
        '<div class = "inform visibility">' +
        '<img src="' + JSON.parse(localStorage.getItem('0').avatar + ' ">' +
        '<p>' +
        'First Name: ' + JSON.parse(localStorage.getItem('0').first_name + '<br>' +
        'Last Name: ' + JSON.parse(localStorage.getItem('0').last_name +
        '</p>' +
        '</div>';

    for (var i = 1; i <localStorage.length; i++) {
        var num = i + 1;
        mainContainer.innerHTML += '<button class="not_active"  onclick = "onClick(' + allInform.id + ')" id = "' + allInform.id + '">' + 'User ' + num + '</button>' +
            '<div class = "inform">' +
            '<img src=" ' + allInform.avatar + ' ">' +
            '<p>' +
            'First Name: ' + allInform.first_name + '<br>' +
            'Last Name: ' + allInform.last_name +
            '</p>' +
            '</div>';
    };
};
